...
void VO2::initialize(int max_frame, const std::string& m_vocfile, int num) {
  ...
  // keyFrameSelection.reset(new KeyFrameSelection(*voc, beta));
...
void VO2::playSequence(const std::vector<std::vector<float>>& poses){
	...
	for(int i = startIndex + 1; i < max_frame; i++) {
		...
		if (loopfound)  { 
			...
			optimizer->addVertex(i, pose, matchingIdx);
	   		...
	   		// if (oposes.size() > 0) {
	   		// 	T = optimizer->cvMat2Eigen(oposes.back().R, oposes.back().t);
	   		// } 
		...
		// g2o::Isometry3D rel_pose = optimizer->cvMat2Eigen(R_key, t_key);
		// R_key = Mat::eye(3,3,CV_64F); t_key = Mat::zeros(3,1,CV_64F);
	    optimizer->addEdge(prevIndex, i, rel_pose, information);
	   	...
	   	if (loopfound && (d_count == 1 || d_count > 100)) {
	   		...
	   		if (h_plots.size() != 0) {
	   			for (auto& ele : h_plots) {
	   				circle(traj, ele, 1, WHITE, 1);
	   			}
	   		}
			...
			t_f = RT.t; R_f = RT.R;
	   		...
	   		for (int i = 0; i < oposes.size(); i++) {
	   			...
	   			circle(traj, o_center,1, DarkRed, 1);
	   			h_plots.push_back(o_center);
   			}
	    } else {
	    	Point2f center = Point2f(int(t_f.at<double>(0)) + 300, int(t_f.at<double>(2)) + 100);
			circle(traj, center ,1, color, 1);
			h_plots.push_back(center);
	    }
		...
		circle(traj, t_center,1, BLUE, 1);
		// Point2f center = Point2f(int(t_f.at<double>(0)) + 300, int(t_f.at<double>(2)) + 100);
		// circle(traj, center ,1, color, 1);
		//plot the information

		//rectangle(traj, Point2f(10, 30), Point2f(550, 50),  Scalar(0,0,0), cv::FILLED);
		putText(traj, estimated_text, Point2f(10,50), cv::FONT_HERSHEY_PLAIN, 1, RED, 1, 5);
		putText(traj, ground_text, Point2f(10,70), cv::FONT_HERSHEY_PLAIN, 1, BLUE, 1, 5);
		putText(traj, loop_detected , Point2f(10,90), cv::FONT_HERSHEY_PLAIN, 1, GREEN, 1, 5);
		putText(traj, optimized_text , Point2f(10,110), cv::FONT_HERSHEY_PLAIN, 1, DarkRed, 1, 5);
		imshow( "Trajectory", traj);
		waitKey(1);
		// }
		//get 3D information of the current frames
		// cv::Mat rcurImg = getImage(right_image_path, i);
		// landmarks.clear();featurePoints.clear();
		// extract_keypoints(i, curImg, rcurImg, landmarks, featurePoints);
